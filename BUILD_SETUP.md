...
`--max-workers=1` — fully serial, not just capped, since AAPT2 runs its
own worker-process pool sized off Gradle's parallelism setting and that
pool isn't bounded by `GRADLE_OPTS` at all) plus `kotlin.daemon.jvm.options`
/ `org.gradle.parallel=false` / `org.gradle.workers.max=1` in
`~/.gradle/gradle.properties` as a backstop, and runs `setup-swap.sh`
first. Without this, the default free Codespaces machine
(`basicLinux32gb`: 2 cores / 8GB RAM) can get **OOM-killed mid-build** —
the whole container (platform-level kill, looks like "the codespace just
stopped" with no build error at all), the Gradle daemon (its own
low-memory self-check, surfaced as "Gradle build daemon disappeared
unexpectedly"), or — seen specifically on `multivendor-app`, which has far
more native modules (Firebase ×2, Stripe, Sentry, Google Sign-In) than
`-rider`/`-store` — mid-AAPT2-shutdown, surfaced as `IllegalStateException:
AAPT Process manager cannot be shut down while daemons are in use`. All
three are the same underlying cause wearing a different stack trace. If it
ever happens again: `gh codespace list` / `gh codespace view -c <name>`
shows the machine size and last-used/updated timestamps — a stop within a
couple minutes of heavy build activity (not ~30 min later, which would be
the idle timeout instead) points at OOM. Remaining levers if it still
recurs even fully serial: close any other terminal running `expo
start`/Metro (`start-app.sh`) first — it's competing for the same 8GB —
and if `multivendor-app` specifically still can't fit, that one app may
genuinely need EAS's cloud build rather than this container's free-tier
machine; `multivendor-rider`/`-store` have far fewer native modules and
are much more likely to fit.
